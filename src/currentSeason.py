...
import numpy as np 
# from main import team_index_26
# from main import a_hat_26
# from main import d_hat_26
...
# Normalize team names to match 2024-25 dataset
name_map_26 = {
...
# Clean and rename columns to match existing format
...
pl_26["date"] = pd.to_datetime(pl_26["date"], dayfirst=True)
...
print(f"Unique teams: {sorted(set(pl_26['home_team']).union(set(pl_26['away_team'])))}")

# manually included the most recent games which aren't included in the dataset
...
# Converts teams to numeric indices for 2025-26
...
remaining_fixtures = pd.DataFrame([
    ...
    ("Bournemouth",           "Leeds"),
    # MD34 continued
    ("Arsenal",               "Nottingham Forest"),  # rescheduled
    # MD35 - April 25-27
    ("Bournemouth",           "Leeds"),
...
def find_mle_26(home_team, away_team):
    home = team_index_26[home_team]
    away = team_index_26[away_team]
    lambda_home = np.exp(a_hat_26[home] - d_hat_26[away] + h_hat_26)
    lambda_away = np.exp(a_hat_26[away] - d_hat_26[home])
    return lambda_home, lambda_away

def simulate_remaining(n=10000):
    ...
    for _ in range(n):
        ...
        # Iterate over exact remaining fixtures
        for _, row in remaining_fixtures.iterrows():
            home = row["HomeTeam"]
            away = row["AwayTeam"]

            lh, la = find_mle_26(home, away)
...
def plot_simulation_results(title_count, top4_count, relegated_count, n, threshold=0.001):
    ...
    def draw_chart(ax, count_dict, title, color):
        # Filter to meaningful probabilities
...
# Print team strengths ranked by attack
